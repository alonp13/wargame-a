...
namespace WarGame {
		...
		uint DemoGame::play() {
			...
			board.move(2, {7,7}, Board::MoveDIR::Down);      // FootSoldier of player 1 moves forward and attacks from {0,1} to {1,1}.
			if (!board.has_soldiers(2)) return 2;
			std::cout << "2------------" << std::endl;
			board.print();

			board.move(1, {0,7}, Board::MoveDIR::Up);      // FootSoldier of player 1 moves forward and attacks from {0,1} to {1,1}.
			if (!board.has_soldiers(2)) return 1;
			std::cout << "3------------" << std::endl;
			board.print();
